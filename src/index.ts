class Node {
  _value: number;
  _leftChild: Node | undefined;
  _rightChild: Node | undefined; 

  constructor(v: number, lc: Node | undefined = undefined, rc: Node | undefined = undefined) {
    this._value = v;
    this._leftChild = lc;
    this._rightChild = rc;
  }
  ...
  set leftChild(lc: Node | undefined) { this._leftChild = lc; }
  set rightChild(lc: Node | undefined) { this._rightChild = lc; }
}
...
class Tree {
  ...
  set root(r: Node | undefined) { this._root = r; }

  buildTree(arr: number[]): Node | undefined {
    if (arr.length === 0) return undefined;
    arr.sort((a, b) => a - b);
    arr = this.removeDuplicates(arr);
    
    const bstRecur = (arr: number[], start: number, end: number) => {
  ...
  prettyPrint(node: Node | undefined, prefix = '', isLeft = true) {
  ...
  includes(v: number) {
    ...
    while (node) {
      ...
      else if (node.value > v) node = node?._leftChild;
  ...
  insertNode(v: number) {
    ...
    let r = this.root;
    if (this.includes(v)) return;
    if (!r) return newNode;

    while (r) {
      if (v > r.value && r.rightChild) r = r.rightChild;
      else if (v < r.value && r.leftChild) r = r.leftChild;
      else break;
    }

    if (v > r.value) r.rightChild = newNode;
    else r.leftChild = newNode;
    ...
    while (node) {
      if (node.value === v) {
        ...
        if (!nodeLeft && !nodeRight) {
          ...
          if (parent) {
            if (fromLeft) parent.leftChild = undefined;
            else parent.rightChild = undefined;
          }  
        } else if (nodeLeft && !nodeRight) {
          // node has one child on the left
          if (parent) {
            if (fromLeft) parent.leftChild = node.leftChild;
            else parent.rightChild = node.leftChild;
          }
        } else if (!nodeLeft && nodeRight) {
          // node has one child on the right
          if (parent) {
            if (fromLeft) parent.leftChild = node.rightChild;
          ...
          const leftMostValue: number | undefined = leftMost.value;
          ...
          if (node.rightChild?.value && leftMostParent.value === node.value) {
            if (leftMost.rightChild) leftMostParent.rightChild = leftMost.rightChild;
            else leftMostParent.rightChild = undefined;
          } else {
            if (leftMost.rightChild) leftMostParent.leftChild = leftMost.rightChild;
            else leftMostParent.leftChild = undefined;
          }
  ...
  levelOrderForEach(callback: (n: number) => void) {
    if (callback === undefined) throw new Error("levelOrderForEach requires callback function");
    const q: Node[] = [];
  ...
  inOrderForEach(callback: (n: number) => void) {
    if (callback === undefined) throw new Error("inOrderForEach requires callback function");
    const q: Node[] = [];
  ...
  preOrderForEach(callback: (n: number) => void) {
    if (callback === undefined) throw new Error("preOrderForEach requires callback function");
    const q: Node[] = [];
  ...
  postOrderForEach(callback: (n: number) => void) {
    if (callback === undefined) throw new Error("postOrderForEach requires callback function");
    const q: Node[] = [];

    const postOrder = (node: Node) => {
  ...
  isBalanced() {
    const checkBalance = (node: Node): boolean => {
      if (!node) return true;
  ...
  rebalance() {
    ...
    const pushToSortedArr = (n: number) => { sortedArr.push(n); } 
